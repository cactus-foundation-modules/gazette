...
import { reorderSeriesPosts } from '@/modules/gazette/lib/db'

type Params = { params: Promise<{ id: string }> }

const Body = z.object({ postIds: z.array(z.string()).min(1) })
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  await reorderSeriesPosts(id, parsed.data.postIds)
